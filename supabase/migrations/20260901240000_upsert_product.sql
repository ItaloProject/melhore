-- Product create/edit was entirely missing from the admin UI — the "Novo
-- Produto" and "Editar" buttons had no handler at all. This RPC saves a
-- product, its category (find-or-create by name) and its variants +
-- initial/edited inventory atomically, mirroring the make_sale/place_order
...
revoke all on function public.upsert_product(uuid, uuid, text, text, text, numeric, numeric, text[], boolean, jsonb) from public;
grant execute on function public.upsert_product(uuid, uuid, text, text, text, numeric, numeric, text[], boolean, jsonb) to authenticated;
